...
function announce() {
    ...
    if (i == 0) i = 5;
    em.getChannelServer().broadcastPacket(tools.CWvsContext.serverNotice(6, "[Event] Field of Judgement will open in " + i + " minutes"));
    i--;
}
...
function mesoDistribution() {
	...
	if (eim.getPlayerCount() > 0) {
	    ...
	    map.broadcastMessage(tools.CWvsContext.serverNotice(6, "[Event] " + winner.getName() + " wins " + meso + " meso"));
...
function start() {
	...
	tools.CWvsContext.serverNotice(6, "[Event] Field of Judgement is now open"));
    var iter = em.getInstances().iterator();
    while (iter.hasNext()) {
	var eim = iter.next();
	if (eim.getPlayerCount() > 0) {
...
function startInstance(eim) {
	...
	while (iter.hasNext()) {
	    ...
	    player.getClient().getSession().write(tools.CWvsContext.getClock(1200));
	}
	var map = eim.getMapInstance(mapId);
...
function playerDead(eim, player) {
    ...
    player.getClient().getSession().write(tools.CWvsContext.serverNotice(1, "You lost with " + eim.getKillCount(player) + " points."));
}
...
function allMonstersDead(eim) {
	...
	winner.getClient().getSession().write(tools.CWvsContext.serverNotice(1, "You win with " + eim.getKillCount(winner) + " points. You will be warped out in 2 minutes."));
	winner.getClient().getSession().write(tools.CWvsContext.getClock(120));
